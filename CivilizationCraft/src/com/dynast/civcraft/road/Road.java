package com.dynast.civcraft.road;
...
public class Road extends Structure {
	...
	private HashMap<BlockCoord, SimpleBlock> oldBlockData = new HashMap<>();
	...
	public void deleteOnDisband() {
		...
		LinkedList<RoadBlock> remove = new LinkedList<>();
	...
	public void fancyDestroyStructureBlocks() {
		for (BlockCoord coord : this.roadBlocks.keySet()) {
			...
			if (rand.nextInt(100) <= 1) {
				FireworkEffect effect = FireworkEffect.builder().with(org.bukkit.FireworkEffect.Type.BURST).withColor(Color.ORANGE).withColor(Color.RED).build();
