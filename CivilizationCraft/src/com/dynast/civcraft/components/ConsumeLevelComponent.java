...
import com.dynast.civcraft.config.*;
...
//	consumeComp.createComponent(this);
	...
	public void createComponent(Buildable buildable, boolean async) {
		...
		if (buildable instanceof Temple) {
			...
			for (ConfigMineLevel lvl : CivSettings.mineLevels.values()) {
				...
				HashMap<Integer, Integer> redstoneAmounts = new HashMap<>();
				redstoneAmounts.put(CivData.REDSTONE_DUST, lvl.amount);
				this.setConsumes(lvl.level, redstoneAmounts);
			}		
		} else if (buildable instanceof Sawmill) {
			for (ConfigSawmillLevel lvl : CivSettings.sawmillLevels.values()) {
				this.addLevel(lvl.level, lvl.count);
				
				HashMap<Integer, Integer> woodAmounts = new HashMap<>();
				woodAmounts.put(CivData.WOOD, lvl.amount);
				this.setConsumes(lvl.level, woodAmounts);
			}				
	    } else if (buildable instanceof Factory) {
			for (ConfigFactoryLevel lvl : CivSettings.factoryLevels.values()) {
				this.addLevel(lvl.level, lvl.count);
				this.setConsumes(lvl.level, lvl.consumes);
			}							
	    } else if (buildable instanceof University) {
			for (ConfigUniverLevel lvl : CivSettings.univerLevels.values()) {
				this.addLevel(lvl.level, lvl.count);
				this.setConsumes(lvl.level, lvl.consumes);
			}
	    } else if (buildable instanceof Pasture) {
	    	for (ConfigPastureLevel lvl : CivSettings.pastureLevels.values()) {
	    		this.addLevel(lvl.level, lvl.amount);
	    		String[] consumes = lvl.consumes.split(",");
	    		Map<Integer, Integer> cons = new HashMap<>();
	    		for (String s : consumes) {
	    			String[] ss = s.split(":");
					cons.put(ItemManager.getId(Material.getMaterial(ss[0])), Integer.valueOf(ss[1]));
				}
	    		this.setConsumes(lvl.level, cons);
	    	}
	    }
	}	
	...
	private String getKey() {
		return getBuildable().getConfigId()+":"+getBuildable().getId()+":"+"levelcount";
	}
