...
public class ConfigBuildableInfo {
	...
	public static void loadConfig(FileConfiguration cfg, String path, Map<String, ConfigBuildableInfo> structureMap, boolean isWonder) {
		...
		for (Map<?, ?> obj : structures) {
			...
			Boolean culturic = (Boolean)obj.get("culturic");
			if (culturic == null || culturic == false) {
				sinfo.culturic = false;
			} else {
				sinfo.culturic = true;
			}
			
			Boolean commercial = (Boolean)obj.get("commercial");
			if (commercial == null || commercial == false) {
				sinfo.commercial = false;
			} else {
				sinfo.commercial = true;
			}
			...
			Boolean national = (Boolean)obj.get("national");
