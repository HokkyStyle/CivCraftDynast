...
import com.dynast.civcraft.config.ConfigSawmillLevel;
...
public class Sawmill extends Structure {
	...
	protected Sawmill(Location center, String id, Town town) throws CivException {
		super(center, id, town);
	}

	public Sawmill(ResultSet rs) throws SQLException, CivException {
		super(rs);
	}
	...
	public void process_sawmill(CivAsyncTask task) throws InterruptedException {
		Result result = null;
		try {
		...
		switch (result) {
		case STARVE:
			CivMessage.sendTown(getTown(), CivColor.Rose+CivSettings.localize.localizedString("var_sawmill_productionFell",getConsumeComponent().getLevel(),CivColor.LightGreen+getConsumeComponent().getCountString()));
			break;
		case LEVELDOWN:
			CivMessage.sendTown(getTown(), CivColor.Rose+CivSettings.localize.localizedString("var_sawmill_lostalvl",getConsumeComponent().getLevel()));
			break;
		case STAGNATE:
			CivMessage.sendTown(getTown(), CivColor.Rose+CivSettings.localize.localizedString("var_sawmill_stagnated",getConsumeComponent().getLevel(),CivColor.LightGreen+getConsumeComponent().getCountString()));
			break;
		case GROW:
			CivMessage.sendTown(getTown(), CivColor.LightGreen+CivSettings.localize.localizedString("var_sawmill_productionGrew",getConsumeComponent().getLevel(),getConsumeComponent().getCountString()));
			break;
		case LEVELUP:
			CivMessage.sendTown(getTown(), CivColor.LightGreen+CivSettings.localize.localizedString("var_sawmill_lvlUp",getConsumeComponent().getLevel()));
			break;
		case MAXED:
			CivMessage.sendTown(getTown(), CivColor.LightGreen+CivSettings.localize.localizedString("var_sawmill_maxed",getConsumeComponent().getLevel(),CivColor.LightGreen+getConsumeComponent().getCountString()));
			break;
		default:
			break;
		}
	...
	public double getBonusHammers() {
		...
		ConfigSawmillLevel lvl = CivSettings.sawmillLevels.get(level);
		double add = lvl.hammers;
