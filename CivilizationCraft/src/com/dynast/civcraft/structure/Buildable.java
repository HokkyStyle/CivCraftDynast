...
import com.dynast.civcraft.interactive.InteractiveBuildCommand;
...
import com.dynast.civcraft.object.MobSpawner;
...
import com.dynast.civcraft.util.AABB;
...
public abstract class Buildable extends SQLObject {
	...
	public static final double SHIFT_OUT = 0.0;
	public static final int MIN_DISTANCE = 7;
	
	private Map<BlockCoord, StructureSign> structureSigns = new ConcurrentHashMap<>();
	private Map<BlockCoord, StructureChest> structureChests = new ConcurrentHashMap<>();
	...
	public int getMaxHitPoints() {
		int maxhp = info.max_hitpoints;
		double rate = 1.0;
		
		if (this instanceof Structure && this.getTown().getStructureTypeCount("s_millitarbase") >= 1) {
			rate += 0.1;
		}
		
		if (this instanceof Wonder && this.getCiv().hasInst("estetic_5")) {
			rate += 0.1;
		}
		
		return (int)Math.round(maxhp * rate);
	}
	...
	public double getHammerCost() {
		double rate = 1.0;
		double hammers = info.hammer_cost;
		
		if (this.getTown().getBuffManager().hasBuff(Buff.RUSH)) {
			rate -= this.getTown().getBuffManager().getEffectiveDouble(Buff.RUSH);
		} else if (this.isTileImprovement() && this.getTown().getBuffManager().hasBuff("buff_mother_tree_tile_improvement_cost")) {
			rate -= this.getTown().getBuffManager().getEffectiveDouble("buff_mother_tree_tile_improvement_cost");
		} else if (this.getTown().getCiv().hasInstitution("chest_3") && this.isStrategic()) {
			rate -= 0.25;
		} else if (this.getTown().getCiv().hasInstitution("estetic_2") && this.isCulturic()) {
			rate -= 0.25;
		} else if (this.getTown().getCiv().hasInstitution("tradition_1") && this.isTileImprovement()) {
			rate -= 0.1;
		} else if (this.getTown().getCiv().hasInstitution("commerce_3") && this.isCommercial()) {
			rate -= 0.25;
		}
	...
	public void buildPlayerPreview(Player player, Location centerLoc, Template tpl) throws CivException, IOException {
		...
		if (!War.isWarTime() && CivSettings.showPreview && centerLoc.getBlockY() > 50){
	...
	protected Location repositionCenter(Location center, String dir, double x_size, double z_size) throws CivException {
		...
		if (this.getConfigId().equals("w_grand_ship_ingermanland") || this.getConfigId().equals("w_flying_dutchman")) {
			...
			if (this.getTemplateYShift() != 0) {				
				loc.setY(62 + this.getTemplateYShift());
			} else {
				loc.setY(62);
			}
		}
	...
	protected void checkBlockPermissionsAndRestrictions(Player player, Block centerBlock, int regionX, int regionY, int regionZ, Location origin) throws CivException {
		...
		if (!isTownHall()) {
			...
			if (!isAllowOutsideTown() && !(this instanceof Wonder) && !isTileImprovement()) {
				if ((cc.getTown() == null || cc.getTown() != this.getTown())) {
					throw new CivException(CivSettings.localize.localizedString("cannotBuild_notInMotherCulture"));
				}
			}
		}
		...
		// TODO return when world generation was default not EWG
//		if (this.getConfigId().equals("s_shipyard") ||
//				this.getConfigId().equals("s_arrowship") ||
//				this.getConfigId().equals("s_scoutship") ||
//				this.getConfigId().equals("s_cannonship") ||
//				this.getConfigId().equals("ti_trade_ship") ||
//				this.getConfigId().equals("w_grand_ship_ingermanland") ||
//				this.getConfigId().equals("w_flying_dutchman") ||
//				this.getConfigId().equals("s_tradeport")) {
...
//		}
		...
		for (int x = 0; x < regionX; x++) {
			for (int y = 0; y < regionY; y++) {
				for (int z = 0; z < regionZ; z++) {
					Block b = centerBlock.getRelative(x, y, z);
					//CultureChunk cc1 = CivGlobal.getCultureChunk(b.getLocation());
					...
					if (this.getConfigId().equals("s_shipyard") ||
							...
							this.getConfigId().equals("w_flying_dutchman") ||
							this.getConfigId().equals("s_tradeport")) {
						if (b.getBiome().equals(Biome.OCEAN) ||
								b.getBiome().equals(Biome.BEACH) ||
								b.getBiome().equals(Biome.STONE_SHORE) ||
								b.getBiome().equals(Biome.SNOWY_BEACH) ||
								b.getBiome().equals(Biome.DEEP_OCEAN) ||
								b.getBiome().equals(Biome.RIVER) ||
								b.getBiome().equals(Biome.FROZEN_OCEAN) ||
								b.getBiome().equals(Biome.FROZEN_RIVER)) {
							wb++;
						}
					}
					...
					if (tc != null && !tc.perms.hasPermission(PlotPermissions.Type.DESTROY, CivGlobal.getResident(player)) && !tc.getTown().getCiv().getLeaderGroup().hasMember(resident)) {
		...
		for (ChunkCoord c : claimCoords) {
			try {
				...
				if (cc != null) {
					this.townChunksToSave.add(TownChunk.townHallClaim(cc.getTown(), c));
				} else {
...
//		}
	...
	public void onDamage(int amount, World world, Player player, BlockCoord coord, BuildableDamageBlock hit) {
		...
		Resident resident = CivGlobal.getResident(player);
			if (resident.isCombatInfo()) {
				player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new ComponentBuilder(CivSettings.localize.localizedString("var_buildable_OnDamageSuccess", hit.getOwner().getDisplayName(), amount, hit.getOwner().hitpoints+"/"+hit.getOwner().getMaxHitPoints())).color(net.md_5.bungee.api.ChatColor.GRAY).create());
	...
	public boolean isCulturic() {
		return info.culturic;
	}
	...
	public void processRegen() {
		...
		if (this.getTown().getCiv().hasInstitution("estetic_5")) {
			if (this instanceof Wonder) {
				regenRate += 2;
			}
		}
...
//	}
	...
	public void validate(Player player) throws CivException {
		if (CivGlobal.getCultureChunk(this.getCenterLocation().getLocation()).getCiv() != this.getCiv()) {
			onDestroy();
		} else {
			TaskMaster.asyncTask(new StructureValidator(player, this), 0);
		}
