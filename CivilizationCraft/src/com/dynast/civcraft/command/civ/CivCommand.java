...
public class CivCommand extends CommandBase {
	...
	public void init() {		
		...
		commands.put("institute", CivSettings.localize.localizedString("cmd_civ_instituteDesc"));
		...
		//commands.put("points", CivSettings.localize.localizedString("cmd_civ_pointsDesc"));
...
//	}
	...
	public void revolution_cmd() throws CivException {
		...
		if (tCiv.hasInstitution("chest_5")) {
			throw new CivException(CivSettings.localize.localizedString("var_cmd_civ_revolutionErrorHasChest5"));
		}
...
//		}
	...
	public void time_cmd() throws CivException {
		...
		cal.setTime(CivGlobal.getNextSpawnBossTime());
		out.add(CivColor.Green+"След. спавн боссов:"+" "+CivColor.LightGreen+sdf.format(cal.getTime()));
		...
		if (player == null || player.hasPermission(CivSettings.MINI_ADMIN) || player.isOp()) {
			...
			EventTimer warTimer = EventTimer.timers.get("spawnbosswither");
		}
	...
	public void institute_cmd() {
		CivInstituteCommand cmd = new CivInstituteCommand();
		cmd.onCommand(sender, null, "institute", this.stripArgs(args, 1));
	}
	...
	public void vote_cmd() throws CivException {
		...
		if (sender instanceof Player) {
			...
			if (!resident.getCiv().getLeaderGroup().hasMember(resident)) {
				CivMessage.sendError(sender, CivSettings.localize.localizedString("cmd_civ_voteNotLeader"));
				return;
			}
