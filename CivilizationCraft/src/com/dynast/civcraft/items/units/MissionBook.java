...
import com.dynast.civcraft.structure.FishingBoat;
import com.dynast.civcraft.structure.Granary;
...
public class MissionBook extends UnitItemMaterial {
	...
	public static double getMissionFailChance(ConfigMission mission, Town town, Player player, int online) {
		int onlineResidents = Math.max(town.getCiv().getOnlineResidents().size(), online);
		...
		if (mission.equals(CivSettings.missions.get("spy_sabotage")) || mission.equals(CivSettings.missions.get("spy_sabotage_wonder"))) {
			if (resident.getTown().getBuffManager().hasBuff("buff_sabotage")) {
				chance *= resident.getTown().getBuffManager().getEffectiveDouble("buff_sabotage");
				CivMessage.send(player, CivColor.LightGray+CivSettings.localize.localizedString("missionBook_sabatoge_buffGoodie"));
			}
		} else if (mission.equals(CivSettings.missions.get("spy_poison_granary"))) {
			if (resident.getTown().getBuffManager().hasBuff("buff_espionage")) {
				chance *= resident.getTown().getBuffManager().getEffectiveDouble("buff_espionage");
				CivMessage.send(player, CivColor.LightGray+CivSettings.localize.localizedString("missionBook_poison_goodie"));
			}			
		} else if (mission.equals(CivSettings.missions.get("spy_steal_treasury"))) {
			if (resident.getTown().getBuffManager().hasBuff("buff_dirty_money")) {
				chance*= resident.getTown().getBuffManager().getEffectiveDouble("buff_dirty_money");
	...
	public static double getMissionCompromiseChance(ConfigMission mission, Town town, int online) {
		int onlineResidents = Math.max(town.getCiv().getOnlineResidents().size(), online);
	...
	public static void performMission(ConfigMission mission, String playerName, int online) {
		Player player;
		try {
		...
		try {
			...
			case "spy_steal_treasury":
				performStealTreasury(player, mission, online);
				break;
			case "spy_incite_riots":
				performInciteRiots(player, mission, online);
				break;
			...
			case "spy_pirate":
				...
				break;
			case "spy_sabotage":
				performSabotage(player, mission, online);
				break;
			case "spy_sabotage_wonder":
				performSabotage(player, mission, online);
				break;
			case "spy_subvert_government":
				performSubertGov(player, mission, online);
				break;
			case "spy_steal_technology":
				performStealTech(player, mission, online);
				break;
			}
	...
	private static boolean processMissionResult(Player player, Town target, ConfigMission mission) {
		return processMissionResult(player, target, mission, 0);
	}

	private static boolean processMissionResult(Player player, Town target, ConfigMission mission, int online) {
		
		int fail_rate = (int)(MissionBook.getMissionFailChance(mission, target, player, online)/* *failModifier */*100);
		int compromise_rate = (int)((MissionBook.getMissionCompromiseChance(mission, target, online))*100);
		Resident resident = CivGlobal.getResident(player);
	...
	private static void performSabotage(Player player, ConfigMission mission, int online) throws CivException {
		Resident resident = CivGlobal.getResident(player);
...
//		}
		...
		if (processMissionResult(player, cc.getTown(), mission, online)) {
	...
	private static void performPosionGranary(Player player, ConfigMission mission) throws CivException {
		...
		Buildable granary = CivGlobal.getNearestBuildable(player.getLocation());
...
//		}
		...
		if (processMissionResult(player, tc.getTown(), mission)) {
			...
			CivGlobal.getSessionDB().add("posiongranary:"+tc.getTown().getName(), value, tc.getTown().getCiv().getId(), tc.getTown().getId(), granary.getId());
	...
	private static void performStealTreasury(Player player, ConfigMission mission, int online) throws CivException {
...
//		}
		...
		if(processMissionResult(player, tc.getTown(), mission, online)) {
			
			double amount = (int)(tc.getTown().getTreasury().getBalance()*0.5);
			if (amount > 0) {
	...
	private static void performSubertGov(Player player, ConfigMission mission, int online) throws CivException {
		Resident resident = CivGlobal.getResident(player);
		if (resident == null || !resident.hasTown()) {
			throw new CivException(CivSettings.localize.localizedString("missionBook_errorNotResident"));
		}
	...
	private static void performInciteRiots(Player player, ConfigMission mission, int online) throws CivException {
		Resident resident = CivGlobal.getResident(player);
		if (resident == null || !resident.hasTown()) {
			throw new CivException(CivSettings.localize.localizedString("missionBook_errorNotResident"));
		}
		...
		ArrayList<SessionEntry> entries = CivGlobal.getSessionDB().lookup("riotsintown:"+tc.getTown().getName());
		if (entries != null && entries.size() != 0) {
			throw new CivException(CivSettings.localize.localizedString("missionBook_riots_already",townhall.getTown().getName()));
		}
		...
		if (processMissionResult(player, tc.getTown(), mission, online)) {
			...
			Random rand = new Random();
			int riots_ticks = rand.nextInt(12) + 12;
			String value = ""+riots_ticks;
			
			CivGlobal.getSessionDB().add("riotsintown:"+tc.getTown().getName(), value, tc.getTown().getCiv().getId(), tc.getTown().getId(), townhall.getId());

			CivMessage.sendSuccess(player, CivSettings.localize.localizedString("var_missionBook_riots_success1"));
			
			CivMessage.global(CivColor.Yellow+CivSettings.localize.localizedString("var_missionBook_riots_alert1",townhall.getTown().getName()));
		}
	}

	private static void performStealTech(Player player, ConfigMission mission, int online) throws CivException {
		Resident resident = CivGlobal.getResident(player);
		if (resident == null || !resident.hasTown()) {
			throw new CivException(CivSettings.localize.localizedString("missionBook_errorNotResident"));
		}
...
//		if (civ.getTechs().size() <= resident.getCiv().getTechs().size()) {
//			throw new CivException(CivSettings.localize.localizedString("var_missionBook_stealTech_errorFewTechs"));
//		}

		if (processMissionResult(player, tc.getTown(), mission, online)) {
			ConfigTech stealedTech;
			LinkedHashSet<ConfigTech> techs = new LinkedHashSet<>();
			Random rand = new Random();
			for (ConfigTech tech : civ.getTechs()) {
				if (tech.era >= civ.getCurrentEra()) {
					if (!(tech.id.equals("tech_enlightenment"))) {
						techs.add(tech);
					}
				}
			...
			while (techs.size() != 5) {
				for (ConfigTech tech : civ.getTechs()) {
					if (tech.era >= civ.getCurrentEra()-1) {
						if (!(tech.id.equals("tech_enlightenment"))) {
							techs.add(tech);
						}
					}
			...
			LinkedList<ConfigTech> techsl = new LinkedList<>();
			techsl.addAll(techs);

			int index = rand.nextInt(techsl.size());
			stealedTech = techsl.get(index);

			if (resident.getCiv().hasTech(stealedTech.id)) {
				CivMessage.sendError(resident, CivSettings.localize.localizedString("var_missionBook_stealTech_pureSpy", stealedTech.name));
				return;
			}

			resident.getCiv().addTech(stealedTech);
			CivMessage.sendSuccess(resident, CivSettings.localize.localizedString("var_missionBook_stealTech_successSteal", stealedTech.name));
		}
	}
}
