...
import com.dynast.civcraft.components.*;
...
import com.dynast.civcraft.interactive.InteractiveBuildableRefresh;
...
import com.dynast.civcraft.threading.sync.SyncUpdateTags;
import com.dynast.civcraft.threading.tasks.BuildAsyncTask;
import com.dynast.civcraft.threading.tasks.BuildUndoTask;
...
import com.dynast.civcraft.util.WorldCord;
...
public class Town extends SQLObject {
	...
	public ConcurrentHashMap<BlockCoord, Structure> structures = new ConcurrentHashMap<>();
...
//	public boolean riots = false;
//	public int riotsTime = 0;
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME+" (" + 
					...
					"`master_civ_id` int(11) NOT NULL DEFAULT 0," + // no longer used.
			...
			SQL.makeCol("defeat_date", "long", TABLE_NAME);
		}
	}
	...
	public void load(ResultSet rs) throws SQLException, InvalidNameException, CivException {
		...
		Long dtime = rs.getLong("defeat_date");
		if (dtime == null || dtime == 0) {
			this.defeat_date = null;
		} else {
			this.setDefeateDate(new Date(dtime));
		}
		...
		String key = "index:"+this.getName();
		ArrayList<SessionEntry> entries = CivGlobal.getSessionDB().lookup(key);

		for (SessionEntry e : entries) {
			int indax = Integer.valueOf(e.value);
			this.setIndex(indax);
		}
	...
	public void saveNow() throws SQLException {
		...
		if (this.defeat_date != null) {
			hashmap.put("defeat_date", this.defeat_date.getTime());
		} else {
			hashmap.put("defeat_date", null);
		}
...
//		}
	...
	public AttrSource getCultureRate() {
		...
		rates.put("Государственный строй", newRate - rate);
		rate = newRate;
		
		ConfigHappinessState state = CivSettings.getHappinessState(this.getHappinessPercentage());
		newRate = rate * state.culture_rate;
		rates.put("Настроение", newRate - rate);
		rate = newRate;

		if (this.getCiv().hasInstitution("estetic_3")) {
			double instRate = CivSettings.publicinsts.get("estetic_3").value;
			rates.put("Общественные институты", instRate);
			rate += instRate;
		}

		if (this.getBuffManager().hasBuff("buff_art_appreciation")) {
			double structures = this.getBuffManager().getEffectiveDouble("buff_art_appreciation");
			rates.put("Структуры", structures);
			rate += structures;
		}
		...
		if (this.getBuffManager().hasBuff("buff_petra_culture")) {
			additional += this.getBuffManager().getEffectiveDouble("buff_petra_culture");
		}
		
		rates.put("Чудеса/Торг. ресурсы", additional);
		rate += additional;
	...
	public AttrSource getCulture() {
		...
		sources.put("Структуры", fromStructures);
		
		double insts = 0;
		if (this.getCiv().hasInstitution("freedom_1")) {
			insts += 50;
		}
		
		total += insts;
		sources.put("Общественные институты", insts);
	...
	public void addAccumulatedCulture(double generated) {
		...
		ConfigCultureLevel clc1 = CivSettings.cultureLevels.get(this.getCultureLevel()-1);

		double amount1 = clc1.amount;
		double amount = clc.amount;

		if (this.getCiv().hasInstitution("estetic_1")) {
			amount *= 0.95;
		}

		double[] instAmount = {amount/2, amount};

		if (index > 1) {
			index = 0;
		}

		if (instAmount[index] <= amount1) {
			instAmount[index] = (amount - amount1) / 2 + amount1;
		}
		
		this.culture += generated;
		this.save();
		if (this.getCultureLevel() != CivSettings.getMaxCultureLevel()) {
			if (this.isCapitol() && this.culture >= instAmount[index]) {
				this.getCiv().points++;
				CivMessage.sendCiv(this.civ, CivSettings.localize.localizedString("var_town_givedpointforints"));
				index++;
			}

			if (this.culture >= amount) {
	...
	public AttrSource getHammerRate() {
		...
		/* Government */
		double newRate = rate * getGovernment().hammer_rate;
		rates.put("Государственный строй", newRate - rate);
		rate = newRate;

		/* Happiness */
		newRate = rate * state.hammer_rate;
		rates.put("Настроение", newRate - rate);
		rate = newRate;


		if (this.getCiv().hasInstitution("freedom_5")) {
			double instRate = CivSettings.publicinsts.get("freedom_5").value;
			rate += instRate;
			rates.put("Общественные институты", instRate);
		}
		...
		rates.put("Случайные события", newRate - rate);
		rate = newRate;
		...
		if (this.motherCiv != null) {
			try {
				...
				rates.put("Штраф захвата", newRate - rate);
				rate = newRate;
	...
	public AttrSource getHammers() {
		...
		sources.put("Торг. ресурсы", wonderGoodies);
		total += wonderGoodies;
		
		double cultureHammers = this.getHammersFromCulture();
		sources.put("Биомы", cultureHammers);
		...
		for (Structure struct : this.structures.values()) {
			...
			if (struct instanceof Sawmill)
			{
				...
				sawmills += sawmill.getBonusHammers(); 
			}
			if (struct instanceof Factory)
			{
				Factory factory = (Factory)struct;
				factoryes += factory.getBonusHammers(); 
			}
			for (Component comp : struct.attachedComponents) {
				if (comp instanceof AttributeBase) {
					AttributeBase as = (AttributeBase)comp;
		...
		total += mines;
		sources.put("Шахты", mines);
		
		total += sawmills;
		sources.put("Лесопилки", sawmills);
		
		total += factoryes;
		sources.put("Фабрика", factoryes);

		total += structures;
		sources.put("Структуры", structures);
		
		
		sources.put("Базовые", this.baseHammers);
...
//		return this.assistantGroup.getName();	
	...
	public void removeResident(Resident resident) {
		...
		if (this.getCiv().getLeaderGroup().hasMember(resident)) {
			this.getCiv().getLeaderGroup().removeMember(resident);
			this.getCiv().getLeaderGroup().save();
		}

		if (this.getCiv().getAdviserGroup().hasMember(resident)) {
			this.getCiv().getAdviserGroup().removeMember(resident);
			this.getCiv().getAdviserGroup().save();
		}
	...
	public int getCultureLevel() {
		...
		if (this.getCiv().hasInstitution("estetic_1")) {
			while (this.culture >= level.amount * 0.95) {
	...
	public void buildWonder(Player player, String id, Location center, Template tpl) throws CivException {
		...
		if (this.getStructureByType("s_barracks") != null) {
			Barracks barracks = (Barracks)this.getStructureByType("s_barracks");
			if (barracks.getTrainingUnit() != null) {
				throw new CivException(CivSettings.localize.localizedString("var_town_buildstructure_unitInProgress"));
			}
		}
...
//		}
		...
		if (this.getCiv().hasInstitution("commerce_2") && (struct instanceof TradeOutpost)) {
			cost /= 2;
		}
		...
		try {
			/*
			 * if the template is null we need to just get the template first. 
	...
	public void demolish(Structure struct, boolean isAdmin) throws CivException {
		...
		try {
			...
			for (StructureSign structureSign : struct.getSigns()) CivGlobal.removeStructureSign(structureSign);
			struct.onDemolish();
			struct.onDemolishFancy();
	...
	public AttrSource getGrowthRate() {
		...
		double instRate = 0.15;
		if (this.getCiv().hasInstitution("tradition_1")) {
			rates.put("Общественные институты", instRate);
			rate += CivSettings.publicinsts.get("tradition_1").value;
		}
		
		if (this.getCiv().hasTechnology("tech_fertilizer")) {
			double techRate = 0.5;
			rates.put("Технологии", techRate);
			rate += techRate;
		}
...
//			if (goodie.getDisplayName().equalsIgnoreCase("��������")) {
//				grapeCount++;
//			}
//		}
...
//		additional += (additionalGrapes*grapeCount);
	...
	public AttrSource getGrowth() {
		...
		sources.put("Биомы", cultureSource);
		total += cultureSource;
		...
		sources.put("Базовая", baseGrowth);
		total += baseGrowth;
	...
	public double getCottageRate() {
		...
		double additional = this.getBuffManager().getEffectiveDouble(Buff.PLEASURE);
		rate += additional;
		
		double additional3 = this.getBuffManager().getEffectiveDouble(Buff.HOTEL_COTTAGE);
		rate += additional3;	
		
		double additional2 = this.getBuffManager().getEffectiveDouble(Buff.PYRAMID_COTTAGE_BONUS);
		rate += additional2;

		if (this.getCiv().hasInstitution("tradition_5")) {
			rate += CivSettings.publicinsts.get("tradition_5").value;
		}
	...
	public double getTradeRate() {
		...
		/* convert this into a 'source' rate so it can be displayed properly. */
		rate += fromStructures;
		
		double additional = this.getBuffManager().getEffectiveDouble(Buff.TRADE);
		rate += additional;
		
		if (this.getBuffManager().hasBuff("buff_machu_trade")) {
			rate += this.getBuffManager().getEffectiveDouble("buff_machu_trade");
		}
		
		if (this.getCiv().hasInstitution("commerce_5")) {
			rate += CivSettings.publicinsts.get("commerce_5").value;
		}
...
/*	public HashSet<BonusGoodie> getEffectiveBonusGoodies() {
	...
	public void removeStructure(Structure structure) {
		...
		for (StructureChest structureChest : structure.getChests()) CivGlobal.removeStructureChest(structureChest);
		for (StructureSign structureSign : structure.getSigns()) CivGlobal.removeStructureSign(structureSign);
		try {
			structure.onDemolish();
		} catch (CivException e) {
			e.printStackTrace();
		}
		structure.onDemolishFancy();
	...
	public void onDefeat(Civilization attackingCiv) {
		...
		CivGlobal.addConqueredTown(this);
		this.defeat_date = new Date();
		this.changeCiv(attackingCiv);
		this.save();
	}
	
	public Date getDefeatDate() {
		return this.defeat_date;
	}

	public void setDefeateDate(Date date) {
		this.defeat_date = date;
	}
...
//		}
	...
	public void capitulate() {
		...
		CivGlobal.removeConqueredTown(this);
		this.setMotherCiv(null);
		this.save();
	...
	public AttrSource getBeakerRate() {
		...
		double newRate = rate*getGovernment().beaker_rate;
		rates.put("Государственный строй", newRate - rate);
		rate = newRate;
		
		ConfigHappinessState state = this.getHappinessState();
		newRate = rate*state.beaker_rate;
		rates.put("Настроение", newRate - rate);
		rate = newRate;
	
		/* Additional rate increases from buffs. */
		/* Great Library buff is made to not stack with Science_Rate */
		double additional = getBuffManager().getEffectiveDouble(Buff.SCIENCE_RATE);
		additional += getBuffManager().getEffectiveDouble("buff_greatlibrary_extra_beakers");
		rate += additional;
		rates.put("Чудеса/Торг. ресурсы", additional);
		
		if (this.getCiv().hasInstitution("rational_1")) {
			double instRate = CivSettings.publicinsts.get("rational_1").value;
			rate += instRate;
			rates.put("Общественные институты", instRate);
		}
	...
	public AttrSource getBeakers() {
		...
		for (Structure struct : this.structures.values()) {
			if (struct instanceof University) {
				...
				univers += univer.getBonusBeakers(); 
			}
			for (Component comp : struct.attachedComponents) {
				if (comp instanceof AttributeBase) {
					AttributeBase as = (AttributeBase)comp;
		...
		beakers += univers;
		sources.put("Университет", univers);
		...
		sources.put("Чудеса/Торг. ресурсы", wondersTrade);
...
/*		double education = 0.0;
	...
	public AttrSource getHappiness() {
		...
		total += townlevel;
		sources.put("Уровень города", townlevel);

		if (this.getCiv().hasInstitution("tradition_2")) {
			double insthappiness = CivSettings.publicinsts.get("tradition_2").value;
			total += insthappiness;
			sources.put("Общественные институты", insthappiness);
		}
		
		/* Grab any sources from buffs. */
		double goodiesWonders = this.buffManager.getEffectiveDouble("buff_hedonism");
		if (this.getBuffManager().hasBuff("buff_machu_happiness")) {
			goodiesWonders += this.buffManager.getEffectiveDouble("buff_machu_happiness");
		}
		sources.put("Чудеса/Баффы", goodiesWonders);
		total += goodiesWonders;
		
		/* Grab happiness from the number of trade goods socketed. */
		int tradeGoods = this.bonusGoodies.size();
		if (this.getCiv().hasInstitution("commerce_1")) {
			tradeGoods *= 3;
		}
		if (tradeGoods > 0) {
			sources.put("Торговые ресурсы", (double)tradeGoods);
		}
		total += tradeGoods;
		
		/* Add in base happiness if it exists. */
		if (this.baseHappy != 0) {
			sources.put("Базовое", this.baseHappy);
			total += baseHappy;
		}
		...
		if (this.getCiv().hasInstitution("chest_1")) {
		    for (Structure struct : this.structures.values()) {
			    if (struct instanceof Barracks) {
			    	structures += 2;
			    }
			    if (struct instanceof MilitaryBase) {
			    	structures += 2;
			    }
		    }
		}
		
		if (this.getCiv().hasInstitution("rational_2")) {
			for (Structure struct : this.structures.values()) {
				...
				if (struct instanceof School) {
					structures +=2;
				}
				if (struct instanceof ResearchLab) {
					structures +=2;
				}
				if (struct instanceof Observatory) {
					structures +=2;
				}
			}
		}
		...
		total += randomEvent;
		sources.put("Случайные события", randomEvent);
	...
	public AttrSource getUnhappiness() {
		...
		if (this.getCiv().hasInstitution("tradition_4")) {
			per_resident *= 0.75;
		}
		...
		sources.put("Жители", (happy_resident+happy_Nonresident+happy_Unresident));
...
//		}
		...
		if (this.getMotherCiv() == null && !this.isCapitol()) {
			...
			sources.put("Расстояние до столицы", distance_unhappy);
		}
		
		/* Add in base unhappiness if it exists. */
		if (this.baseUnhappy != 0) {
			sources.put("Базовое", this.baseUnhappy);
	...
	public boolean processRiots() {
		String key = "riotsintown:"+this.getName();
		ArrayList<SessionEntry> entries;
		entries = CivGlobal.getSessionDB().lookup(key);
		int max_riots_ticks = -1;
		for (SessionEntry entry : entries) {
			int next = Integer.valueOf(entry.value);
			
			if (next > max_riots_ticks) {
				max_riots_ticks = next;
			} 
		}
		
		if (max_riots_ticks > 0) {
			CivGlobal.getSessionDB().delete_all(key);
			max_riots_ticks--;
			
			if (max_riots_ticks > 0)
				CivGlobal.getSessionDB().add(key, ""+max_riots_ticks, this.getCiv().getId(), this.getId(), this.getTownHall().getId());
	
			CivMessage.sendTown(this, CivColor.Rose+CivSettings.localize.localizedString("effectEvent_riots_notEnded", max_riots_ticks));
			return true;
		}
		return false;
	}	
	...
	public void refreshNearestBuildable(Resident resident) throws CivException {
		if (!this.getMayorGroup().hasMember(resident) && !this.getCiv().getLeaderGroup().hasMember(resident)) {
		...
		if (buildable.getCiv() != this.getCiv()) {
